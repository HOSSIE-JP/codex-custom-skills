...
- 第2話以降は、シナリオに合わせた話数固有イベントスチルを最低3枚計画し、人間が生成・登録する
...
各話につき最低3枚を計画し、人間がChatGPT Imageで生成してPCE Game Editorへ登録します。標準IDと役割は次のとおりです。
...
- スキルはイベントスチル画像を生成しない
- スキルは、人間が別途ChatGPT Imageへ投入できる完成プロンプトを1枚ごとに作る
- 完成プロンプトには、参照画像、構図、表情、ポーズ、小物、背景、28:17安全領域、禁止事項を含める
- 状態は `PROMPT_READY / IMAGE_PENDING_HUMAN` とする
- 人間が参照画像を添付し、別途ChatGPT Imageで画像を生成する
- 人間がPCE Game EditorのImage画面でクロップ、リサイズ、減色、パレット生成、asset登録を行う
- スキルは画像生成済み、224×136済み、16色済み、登録済みと偽らない
- 画像の生成、編集、変換、画像検査へPython、Pillow、OpenCV、matplotlib、NumPy、SVG、HTML canvas、プログラム描画を使わない
...
- プロンプト引き渡し票に、人間がChatGPT Imageへ添付する既存sprite PNGまたは設定画を列挙する
- 髪型、髪色、服装、配色、年齢感を既存立ち絵へ合わせる
- 表情とポーズはシナリオの感情に合わせて変えてよい
- キャラクターを別人へ見せる大幅な衣装変更や体格変更はしない

完成プロンプトでは、重要人物と小物を中央寄りにした28:17向けの安全な構図を指定します。人間がChatGPT Imageで生成した後、PCE Game EditorのImage画面で224×136へクロップまたはリサイズし、内部変換で減色・パレット生成を行います。スキルは画像を生成・補正・検査せず、縦横比の変形も指示しません。
  ...
  "proposedAssets": [
    {
      ...
      "status": "PROMPT_READY / IMAGE_PENDING_HUMAN",
      "referenceImages": [
        "キャラクター設定_部長.png",
        "キャラクター設定_レン.png"
      ],
      "cropSafeArea": "重要人物と小物を中央80%へ配置し、28:17クロップで切らない",
      "imagePrompt": "別のChatGPT Image会話へそのまま貼れる自己完結した完成プロンプト",
      "acceptanceChecks": [
        "参照キャラクターと同一人物に見える",
        "文字・UI・透かしがない",
        "PCE減色後も表情と小物が読める"
...
このJSONは画像プロンプト引き渡し計画であり、`pce-assets.json` へそのまま追加するmanifestではありません。スキルは `PROMPT_READY / IMAGE_PENDING_HUMAN` までを作成します。人間が参照画像を添付してChatGPT Imageを実行し、PCE Game Editorで224×136へ変換してbackground assetへ登録します。更新後の `pce-assets.json` に正しいIDが存在することを確認してから完成スクリプトへ反映します。
